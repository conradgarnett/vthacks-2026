...
class AllergyWatch:
    ...
    def compose_alert(self, alert: Alert, frame_jpeg: bytes | None = None):
        ...
        subject = f"VisionOS alert: possible {alert.names} exposure"
        if profile.name:
            subject += f" for {profile.name}"
        where = alert.place or "not known (no place recognized recently)"
        body = (
            f"This is an automated alert from VisionOS, the assistive glasses worn by {who}.\n\n"
    ...
    def compose_correction(self, alert: Alert):
        ...
        profile = self.profile
        who = profile.name or "the wearer"
        subject = f"VisionOS correction: the {alert.names} alert was a false alarm"
        body = (
            f"{who} has said that the VisionOS alert sent at {when_text(alert.at)} about "
