    ...
    async def describe(
        ...
        try:
            async with self._client.messages.stream(
                model=self._settings.mediglasses_model,
                ...
                output_config={"effort": self._settings.mediglasses_effort},
...
def build_provider(
    ...
    if settings.vision_provider == "nvidia":
        ...
        if not settings.nvidia_api_key:
            log.warning(
                ...
                "mode. Put the key in med-i-glasses/.env."
            )
            return local
    ...
    log.info("Vision provider: Claude (%s)", settings.mediglasses_model)
