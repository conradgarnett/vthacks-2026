...
# The macOS faces come first so numbers measured there are unchanged. Off
# macOS the same faces are taken from a Windows installation, and the
# bundled Open Sans is the last resort so the corpus renders anywhere. A
# label set in a different face is a different label: numbers compare
# only within one platform.
_FALLBACK_FONTS = {
    _LABEL_FONT: ["C:/Windows/Fonts/ARIALN.TTF", "C:/Windows/Fonts/arial.ttf"],
    _BOLD_FONT: ["C:/Windows/Fonts/arialbd.ttf"],
}
_LAST_RESORT_FONTS = [
    "/System/Library/Fonts/Helvetica.ttc",
    str(FONT_DIR / "OpenSans-Variable.ttf"),
]


def _font(path: str, px: int):
    for candidate in [path, *_FALLBACK_FONTS.get(path, []), *_LAST_RESORT_FONTS]:
        try:
