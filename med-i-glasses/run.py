#!/usr/bin/env python3
"""One command to run Med-i-Glasses on macOS, Linux or Windows.
...
def scanner_report() -> list[str]:
    ...
    if mail == "yes":
        ...
        out.append("      ALERT_SMTP_PASSWORD in med-i-glasses/.env turn it on)")
    out.append(
    ...
    out.append(
        ...
        else "Voice: the browser's own (ELEVENLABS_API_KEY in med-i-glasses/.env for a better one)"
    )
    return out
...
def main() -> int:
    ...
    if has_claude:
        ...
        say("        ANTHROPIC_API_KEY=... to med-i-glasses/.env for full answers.")
