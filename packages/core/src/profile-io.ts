import { SensoryProfileSchema, SignedProfileSchema, canonicalJson, type SensoryProfile, type SignedProfile } from '@sense/protocol';
...
export async function exportProfile(profile: SensoryProfile, keys: KeyPair): Promise<SignedProfile> {
 ...
 */
export async function importProfile(raw: unknown, expectedPublicKey?: string): Promise<SensoryProfile> {
  ...
  const ok = await verifyBytes(await importSpki(file.publicKey), new TextEncoder().encode(canonicalJson(file.profile)), file.signature);
