...
- D28 — Process note: from M7 on, verify is run with pipefail and its real exit code is checked (an earlier shell pattern reported the wrong process's status; one lint failure was committed and fixed in the next commit, m7-done re-tagged).
